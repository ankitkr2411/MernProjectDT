...
const SingleProductSaree = () => {
  ...
  let {single_Product, singleProduct, isSingleLoading} = useProductContext();
...
colour = colors[ind].color;
  ...
  if(user_Data.wishlist.length != 0)
  {
    const wishcheck = user_Data.wishlist.filter((currElem) => {
      return currElem.id == id && currElem.color == colour
    })
...
let discount = Math.ceil(((mrp - colors[ind].price)/mrp)*100);
    ...
    return (
      ...
      <>
        ...
        <Wrapper border={ind} image={imgind} >
...
export default SingleProductSaree